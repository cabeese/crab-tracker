...
/* ======================= PIN_D Variables ======================= */
// Masks off digital pins 0, 1, 2, and 7.
...
void setup (void) {
  ...
  // will disable outputs  on pins 9 and 10 which disables PWM, 
  // this allows use of SPI and timer1 at the same time
  TCCR1A = 0;  
  TCCR1B = 1;// start timer 1 
  //  TCCR1C = 0;  // I'm unsure if this is needed
  TCNT1 = 0; // clear out timer1
  ...
  /* PIN_D Setup - Sets D pins  3,4,5,6 to input */
  DDRD = 0b10000111;
...
void loop (void){
  while(1){
    ...
    if (xorpins) { // a hydrophone heard something 
      TCCR1B = 0;    //stop the timer 
      volatile  uint16_t temp_timer = TCNT1;  //store passed ticks  
      TCCR1B = 1; // restart the timer
      volatile unsigned long ticks = ((tmr1_overflow << 16) + (unsigned long)temp_timer); 

      // this assignment may be unneeded
      time_elapsed = ticks;

      // stores high pins and timestamp
      output[bb_end].pinvals = (pinval >> 3) & 0b00001111; 
