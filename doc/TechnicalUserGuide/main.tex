...
\subsection{Wiring Diagrams}\label{sec:wiring}

This section details the wiring for the following devices:
\begin{itemize}
\item Raspberry Pi 3B
\item AdaFruit RaspberryPi 7" Touch Screen
\item Arduino Nano
\item Receiver hardware
\end{itemize}
...
\matrix[wire board,arduino, matrix anchor=north, right = 9cm of Pi.north] (Arduino){
	D02 &  2 & GND           \\
	D03 &  3 & Hydrophone    \\
	D04 &  4 & Hydrophone    \\
	D05 &  5 & Hydrophone    \\
...
\draw[color=red] % SPI connections
...
;
\draw
	(Pi-5-2) -- +(3cm, 0)   |- (Arduino-1-2) % GND
;

%\draw
%	(Pi-1-2) -- +(2cm, 1cm) -| (Arduino-7-2)
%;

%\draw
%	node at (0,0) []          {\Large $\circ$}
%	node at (0,1) [fill=red]  {\Large $\circ$}
%	node at (1,1) [fill=blue] {\Large $\circ$}
%;
%\draw[help lines] (0,0) grid (8,-8);
%
%\draw (-1,0) rectangle (2,-4); % box for the Pi
%
%\foreach \x in {1,...,24}
%	\node at (\intcalcMod{\x}{2}, \intcalcDiv{\x+1}{2}) (pi-\x) [pin] {\x};

% Draw SPI pins
%\foreach \x in {19,21,23}
%	\node at (0, \x/2 - 12.5) (pi-\x) [pin] {\x};
%\foreach \x in {20,22,24}
%	\node at (1, \x/2 - 13) (pi-\x) [pin] {\x};


%\node at (-1,0)   (pi-23)  [pin] {23};
%\node at ( 0,0)   (pi-24)  [pin] {24};
%\node at (-1,-1)  (pi-25)  [pin] {25};
%\node at ( 0,-1)  (pi-26)  [pin] {26};
%\node at (-1,-2)  (pi-5) [pin] { 5};
%\node at ( 0,-2)  (pi-6) [pin] { 6};
%\node at (-1,-3)  (pi-7) [pin] { 7};
%\node at ( 0,-3)  (pi-8) [pin] { 8};
%\node at (-1,-4)  (pi-9) [pin] { 9};
%\node at ( 0,-4)  (pi-p10)  [pin] {10};
%\node at (-1,-5)  (pi-p11)  [pin] {11};
%\node at ( 0,-5)  (pi-p12)  [pin] {12};
%\node at (-1,-6)  (pi-p13)  [pin] {13};
%\node at ( 0,-6)  (pi-p14)  [pin] {14};

%\node at (3,0)    (ard-ss)  [pin] {SS};

%\draw[->,red] (pi-p10) -| node[near start,below] {foo} (ard-ss);
%\draw[->,red] (pi-p10) .. controls +(up:1cm) and +(left:1cm) .. node[above, sloped] {label} (ard-ss);
%\draw[->,red,thick] (pi-p10) .. controls (1,1) and (2,1) .. (ard-ss);


\end{tikzpicture}
\end{center}
\caption{A wiring diagram for the computer hardware.}
\label{fig:wiring}
\end{figure}
